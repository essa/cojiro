...
Then /^I (should|should not) see the text:? "([^"]*)" in the (thread|threads list|navbar|statbar)$/ do |expectation, text, class_tag|
  within(:css, (class_tag == 'navbar') ? '#navbar' : ".#{class_tag.gsub(' ', '_')}") do
    page.send(expectation.gsub(' ','_'),have_content(text))
  end
end
